...
const createUsers = async function(req,res){
  let data = req.body
  let usersData = await userModel.create(data)
...
const loginUsers = async function (req,res){  console.log("from line 110")
  ...
  if(!user)
   ...
   let token = jwt.sign(
    ...
    "juju plutonium very very secret key"
   );
   res.setHeader("x-auth-token", token);
   res.send({status:true,token:token})
}
...
const updateUser = async function(req,res){
  ...
  let userData = req.body
  let updatedUser = await userModel.findOneAndUpdate({_id:userId1},userData)
  res.send({status:updatedUser,data:updatedUser})
}

// delete user

const docDelete = async function(req,res){
  let userId1 = req.params.userId

  if(!userId1)
  return res.send({msg:"user doesn't exist"})

  let deleteUser = await userModel.updateOne({_id:userId1},{$set:{isDeleted:true}},{upsert:true})
