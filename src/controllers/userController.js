...
const deleteUser = async function(req,res){
 try{ let user = req.params.userId
  if(!user)
  return res.status(404).send({status:false,msg:"user_Id is must required"})
  let dltusr = await userModel.findOneAndUpdate({_id:user},{$set:{isDeleted:true}},{upsert:true})
  res.status(200).send({msg:dltusr})}
  catch(err){
    res.status(500).send({msg:err.message})
  }
}
